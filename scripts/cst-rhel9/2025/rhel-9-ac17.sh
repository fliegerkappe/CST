...
title7b="Checking with: 'sudo /usr/sbin/sshd -dd 2>&1 | awk '/filename/ {print \$4}' | tr -d '\\\\r' | tr '\\\\n' ' ' | sudo grep -iH '^\s*rekeylimit'"
...
loglevel="$(/usr/sbin/sshd -dd 2>/dev/null | awk '/filename/ {print toupper($4)}' | tr -d '\r' | tr '\n' ' ' | xargs grep -iH '^\s*loglevel')"
...
then
  ...
  then
    fail=0
    echo -e "${NORMAL}RESULT:    ${BLD}$loglevel${NORMAL}"
  fi
else
